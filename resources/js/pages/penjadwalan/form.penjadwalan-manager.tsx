...
export default function PenjadwalanForm() {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            ...
            href: route(isEdit ? 'penjadwalan.edit' : 'penjadwalan.create', isEdit ? penjadwalan.id_penjadwalan : ''),
        },
    ];
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        if (isEdit) {
            router.put(route('penjadwalan.update', penjadwalan.id_penjadwalan), values, {
                preserveScroll: true,
            ...
            router.post(route('penjadwalan.store'), values, {
                preserveScroll: true,
                preserveState: false, // Force fresh data load
                onSuccess: () => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                <div className="space-between flex items-center justify-between">
                    ...
                    <CButton type="primary" className="md:w-24" onClick={() => router.visit(route('penjadwalan.index'))}>
                        Kembali
                    </CButton>
                </div>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            render={({ field }) => {
                                ...
                                return (
                                    <FormItem>
                                        ...
                                        {isEdit ? (
                                            <div className="bg-muted rounded border px-3 py-2">{namaEvent || '-'}</div>
                                        ) : (
                                            <Select
                            ...
                            render={({ field }) => {
                                ...
                                return (
                                    <FormItem>
                                        ...
                                        {isEdit ? (
                                            <div className="bg-muted rounded border px-3 py-2">{namaKategori || '-'}</div>
                                        ) : (
                                            <Select
                                                value={field.value > 0 ? field.value.toString() : ''}
                                                onValueChange={(value) => field.onChange(parseInt(value))}
                        ...
                        <FormField
                            ...
                            render={({ field }) => {
                                ...
                                const jumlahTerdaftar =
                                    penjadwalan &&
                                    typeof penjadwalan === 'object' &&
                                    'jumlahTerdaftar' in penjadwalan &&
                                    typeof penjadwalan.jumlahTerdaftar === 'number'
                                ...
                                return (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input type="number" placeholder="Masukkan kuota peserta" {...field} onChange={handleKuotaChange} />
                                        </FormControl>
